...
``main`` should always be green and a potential release candidate. ``pytest-rich`` follows
semantic versioning, so given that the current version is ``X.Y.Z``, to find the next version number
one needs to look at the ``CHANGELOG.md`` file:
...
#. Commit and push the branch to ``origin`` and open a PR.

#. Once the PR is **green** and **approved**, start the ``deploy`` workflow:

   .. code-block:: console

        gh workflow run deploy.yml -R Jamorim-esss/pytest-rich --ref release-VERSION --field version=VERSION
